...
//  WarFly
//
//  Created by Alex Alexandro on 06.03.2021.
//
...
//      spawnEnemy(count: 5)
    ...
    fileprivate func spawnClouds() {
        ...
        let spawnCloudSequnece = SKAction.sequence([spawnCloudWait, spawnCloudAction])
        let spawnCloudForever = SKAction.repeatForever(spawnCloudSequnece)
        run(spawnCloudForever)
    }
    
    fileprivate func spawnIslands() {
        let spawnIslandWait = SKAction.wait(forDuration: 2)
        let spawnIslanddAction = SKAction.run {
            let island = Island.populate(at: nil)
            self.addChild(island)
        }
        
        let spawnIslandSequnece = SKAction.sequence([spawnIslandWait, spawnIslanddAction])
        let spawnIslandForever = SKAction.repeatForever(spawnIslandSequnece)
